...
static uint32 pci_config_read(const uint8 bus, const uint8 device, const uint8 function, const uint8 offset) {
	...
	for (uint8 bus = 0; bus < 8; bus++) {
		for (uint8 device = 0; device < 32; device++) {
			for (uint8 function = 0; function < 8; function++) {
				const uint32 vendor_device = pci_config_read(bus, device, function, 0x00);
				if ((vendor_device & 0xFFFF) == 0xFFFF) continue;
				const uint32 class_reg = pci_config_read(bus, device, function, 0x08);
